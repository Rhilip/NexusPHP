...
end_frame();
unset($ppl);
$res = \NexusPHP\Components\Database::query("SELECT * FROM language ORDER BY trans_state") or sqlerr();
...
end_frame();
unset($ppl);
$res = \NexusPHP\Components\Database::query("SELECT * FROM stylesheets ORDER BY id") or sqlerr();
while ($arr = mysqli_fetch_assoc($res)) {
