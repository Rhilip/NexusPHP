...
if (is_valid_id($remove)) {
    \NexusPHP\Components\Database::query("DELETE FROM bans WHERE id=".\NexusPHP\Components\Database::real_escape_string($remove)) or sqlerr();
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && get_user_class() >= UC_ADMINISTRATOR) {
    ...
    $comment = \NexusPHP\Components\Database::escape($comment);
    $added = \NexusPHP\Components\Database::escape(date("Y-m-d H:i:s"));
    \NexusPHP\Components\Database::query("INSERT INTO bans (added, addedby, first, last, comment) VALUES($added, ".\NexusPHP\Components\Database::real_escape_string($CURUSER[id]).", $firstlong, $lastlong, $comment)") or sqlerr(__FILE__, __LINE__);
...
$res = \NexusPHP\Components\Database::query("SELECT * FROM bans ORDER BY added DESC") or sqlerr();
