...
function autoclean()
{
    ...
    $res = \NexusPHP\Components\Database::query("SELECT value_u FROM avps WHERE arg = 'lastcleantime'");
    $row = mysqli_fetch_array($res);
    if (!$row) {
        \NexusPHP\Components\Database::query("INSERT INTO avps (arg, value_u) VALUES ('lastcleantime',$now)") or sqlerr(__FILE__, __LINE__);
        return false;
    }
    $ts = $row[0];
    ...
    \NexusPHP\Components\Database::query("UPDATE avps SET value_u=$now WHERE arg='lastcleantime' AND value_u = $ts") or sqlerr(__FILE__, __LINE__);
