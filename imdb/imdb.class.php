<?php
 ...
 class imdb extends imdb_config
 {
     ...
     public $main_genres = "";
     public $main_tagline = "";
     public $main_plotoutline = "";
     public $main_comment = "";
     public $main_alttitle = "";
     public $main_colors = "";
     ...
     public $credits_cast = "";
     public $credits_director = "";
     public $credits_writing = "";
     public $credits_producer = "";

     public $main_director = "";
     public $main_credits = "";
     public $main_photo = "";
     public $main_country = "";
     public $main_alsoknow = "";
     public $main_sound = "";

     public $info_excer;
  
     public $similiar_movies = array(array('Name' => '', 'Link' => '', 'Local' => ''));	// no Thumbnail here, since it works different from last.fm, douban
     public $extension = array('Title', 'Credits', 'Amazon', 'Goofs', 'Plot', 'Comments', 'Quotes', 'Taglines', 'Plotoutline', 'Trivia', 'Directed');
   ...
   */
     public function similiar_movies()
     {
      ...
      */
     public function cachestate($target = "")
     {
         if (strlen($this->imdbID) != 7) {
     ...
     * @param $target array
     */
     public function preparecache($target = "", $retrive_similiar = false)
     {
         $ext_arr = isset($target) ? $target : $this->extension;
         foreach ($ext_arr as $ext) {
             $tar_ext = array($ext);
    ...
    case "Taglines": $urlname="/taglines"; break;
      ...
      * @method purge
      */
     public function purge($explicit = false)
     {
      ...
      * @method purge
      */
     public function purge_single($explicit = false)
     {
         if (is_dir($this->cachedir)) {
             $thisdir = dir($this->cachedir);
...
#    $rate_s = strpos ($this->page["Title"], '/ratings">');
             ...
             if (strpos($this->page["Title"], "awaiting 5 votes")) {
                 return false;
             }
...
//    $this->main_votes = substr ($this->page["Title"], $vote_s, $vote_e - $vote_s);
    ...
    $genre_e = strpos($this->page["Title"], "/rg/title-tease/", $genre_s);
      ...
      */
     public function get_table_rows($html, $table_start)
     {
      ...
      * @return mixed rows (FALSE if table not found, array[0..n] of strings otherwise)
      */
     public function get_table_rows_cast($html, $table_start)
     {
...
#     $row_x = strpos( $temp, '<td valign="middle"' );
      ...
      */
     public function get_row_cels($row)
     {
         $cel_s = 0;
         $cel_e = 0;
         $endrow = strlen($row);
      ...
      * @return string
      */
     public function get_imdbname($href)
     {
         if (strlen($href) == 0) {
             return $href;
         }
...
#    $tag_s = strpos ($this->page["Title"], "http://ia.imdb.com/media",$tag_s);
      ...
      */
     public function photo_localurl()
     {
    ...
    $country_e = strpos($this->page["Title"], "</div>", $country_s);
     ...
     public function alsoknow()
     {
         if ($this->main_alsoknow == "") {
             ...
             if ($ak_s == 0) {
                 $ak_s = strpos($this->page["Title"], "Alternativ:");
             }
             if ($ak_s == 0) {
                 return array();
             }
             ...
             for ($i=0; $i< count($alsoknow_arr); $i++) {
                 if (strpos($alsoknow_arr[$i], "href=")!==false) {
                     continue;
                 } // link to more AKAs
                 ...
                 if (strlen($alsoknow_arr[$i])>0) {
                     ...
                     for ($k=1;$k<$elems;++$k) {
                         ...
                         if (!isset($ak_temp["year"])) {
                             ...
                             if (isset($ak_temp["comment"]) && !empty($ak_temp["comment"])) {
                                 $ak_temp["comment"] .= " $var";
                             } else {
                                 $ak_temp["comment"] = $var;
                             }
                     ...
                     if (!isset($ak_temp["country"])) {
                         $ak_temp["country"] = "";
                     }
                     if (!isset($ak_temp["comment"])) {
                         $ak_temp["comment"] = "";
                     }
     ...
     { // patch by Brian Ruth not yet tested (by me...)
