...
$res = \NexusPHP\Components\Database::query("SELECT username FROM users WHERE id = ".\NexusPHP\Components\Database::real_escape_string($id)) or sqlerr();
...
$res = \NexusPHP\Components\Database::query("SELECT invites FROM users WHERE id = ".\NexusPHP\Components\Database::real_escape_string($id)) or sqlerr();
...
if ($type == 'new') {
    ...
    $rel = \NexusPHP\Components\Database::query("SELECT COUNT(*) FROM users WHERE invited_by = ".\NexusPHP\Components\Database::real_escape_string($id)) or sqlerr(__FILE__, __LINE__);
    $arro = mysqli_fetch_row($rel);
    $number = $arro[0];

    $ret = \NexusPHP\Components\Database::query("SELECT id, username, email, uploaded, downloaded, status, warned, enabled, donor, email FROM users WHERE invited_by = ".\NexusPHP\Components\Database::real_escape_string($id)) or sqlerr();
    ...
    if ($CURUSER[id] == $id || get_user_class() >= UC_SYSOP) {
        $pendingcount = number_format(\NexusPHP\Components\Database::count("users", "WHERE  status='pending' AND invited_by=$CURUSER[id]"));
        if ($pendingcount) {
    ...
    $rul = \NexusPHP\Components\Database::query("SELECT COUNT(*) FROM invites WHERE inviter =".\NexusPHP\Components\Database::real_escape_string($id)) or sqlerr();
    $arre = mysqli_fetch_row($rul);
    $number1 = $arre[0];


    $rer = \NexusPHP\Components\Database::query("SELECT invitee, hash, time_invited FROM invites WHERE inviter = ".\NexusPHP\Components\Database::real_escape_string($id)) or sqlerr();
