...
if ($pollid) {
    $res = \NexusPHP\Components\Database::query("SELECT * FROM polls WHERE id = ".\NexusPHP\Components\Database::escape($pollid)." LIMIT 1") or sqlerr();
 ...
 "<td class=colhead align=center><nobr>".$lang_polloverview['col_id']."</nobr></td><td class=colhead><nobr>".$lang_polloverview['col_added']."</nobr></td><td class=colhead><nobr>".$lang_polloverview['col_question']."</nobr></td></tr>\n");
    ...
    print("</table>\n");
    $count = \NexusPHP\Components\Database::count("pollanswers", "WHERE pollid = ".\NexusPHP\Components\Database::escape($pollid)." AND selection < 20");
    ...
    if ($count == 0) {
        ...
        $res2 = \NexusPHP\Components\Database::query("SELECT pollanswers.*, users.username FROM pollanswers LEFT JOIN users ON pollanswers.userid = users.id WHERE pollid = ".\NexusPHP\Components\Database::escape($pollid)." AND selection < 20 ORDER BY username ASC ".$limit) or sqlerr();
        print($pagertop);
        ...
        while ($useras = mysqli_fetch_assoc($res2)) {
    ...
    $res = \NexusPHP\Components\Database::query("SELECT id, added, question FROM polls ORDER BY id DESC") or sqlerr();
    if (mysqli_num_rows($res) == 0) {
