...
$count = \NexusPHP\Components\Database::count("reports");
if (!$count) {
...
$reportres = \NexusPHP\Components\Database::query("SELECT * FROM reports ORDER BY dealtwith ASC, id DESC $limit");

while ($row = mysqli_fetch_array($reportres)) {
    if ($row['dealtwith']) {
    ...
    switch ($row['type']) {
        case "torrent":
        {
            ...
            $res = \NexusPHP\Components\Database::query("SELECT id, name FROM torrents WHERE id=".\NexusPHP\Components\Database::escape($row['reportid']));
        ...
        case "user":
        {
            ...
            $res = \NexusPHP\Components\Database::query("SELECT id FROM users WHERE id=".\NexusPHP\Components\Database::escape($row['reportid']));
            if (mysqli_num_rows($res) == 0) {
        ...
        case "offer":
        {
            ...
            $res = \NexusPHP\Components\Database::query("SELECT id, name FROM offers WHERE id=".\NexusPHP\Components\Database::escape($row['reportid']));
            if (mysqli_num_rows($res) == 0) {
...
/*
        ...
        {
            $type = "Request";
            $res = \NexusPHP\Components\Database::query("SELECT id, request FROM requests WHERE id=".\NexusPHP\Components\Database::escape($row['reportid']));
            if (mysqli_num_rows($res) == 0)
...
*/
        case "post":
        {
            ...
            $res = \NexusPHP\Components\Database::query("SELECT topics.id AS topicid, topics.subject AS subject, posts.userid AS postuserid FROM topics LEFT JOIN posts ON posts.topicid = topics.id WHERE posts.id=".\NexusPHP\Components\Database::escape($row['reportid']));
            if (mysqli_num_rows($res) == 0) {
        ...
        case "comment":
        {
            ...
            $res = \NexusPHP\Components\Database::query("SELECT id, user, torrent, offer FROM comments WHERE id=".\NexusPHP\Components\Database::escape($row['reportid']));
            if (mysqli_num_rows($res) == 0) {
        ...
        case "subtitle":
        {
            ...
            $res = \NexusPHP\Components\Database::query("SELECT id, torrent_id, title FROM subs WHERE id=".\NexusPHP\Components\Database::escape($row['reportid']));
            if (mysqli_num_rows($res) == 0) {
