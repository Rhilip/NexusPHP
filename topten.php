...
if (!$Cache->get_page()) {
    ...
    if ($type == 1) {
        ...
        if ($limit == 10 || $subtype == "ul") {
            $order = "uploaded DESC";
            $r = \NexusPHP\Components\Database::query($mainquery . " ORDER BY $order " . " LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "uls") {
            ...
            $r = \NexusPHP\Components\Database::query($mainquery . $extrawhere . " ORDER BY $order " . " LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "act") {
            $r = \NexusPHP\Components\Database::query("SELECT t.*, (t.size * t.times_completed + SUM(p.downloaded)) AS data FROM torrents AS t LEFT JOIN peers AS p ON t.id = p.torrent WHERE p.seeder = 'no' GROUP BY t.id ORDER BY seeders + leechers DESC, seeders DESC, added ASC LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "sna") {
            $r = \NexusPHP\Components\Database::query("SELECT t.*, (t.size * t.times_completed + SUM(p.downloaded)) AS data FROM torrents AS t LEFT JOIN peers AS p ON t.id = p.torrent WHERE p.seeder = 'no' GROUP BY t.id ORDER BY times_completed DESC LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "mdt") {
            $r = \NexusPHP\Components\Database::query("SELECT t.*, (t.size * t.times_completed + SUM(p.downloaded)) AS data FROM torrents AS t LEFT JOIN peers AS p ON t.id = p.torrent WHERE p.seeder = 'no' AND times_completed > 0 GROUP BY t.id ORDER BY data DESC, added ASC LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "bse") {
            $r = \NexusPHP\Components\Database::query("SELECT t.*, (t.size * t.times_completed + SUM(p.downloaded)) AS data FROM torrents AS t LEFT JOIN peers AS p ON t.id = p.torrent WHERE p.seeder = 'no' AND seeders >= 5 GROUP BY t.id ORDER BY seeders / leechers DESC, seeders DESC, added ASC LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "wse") {
            $r = \NexusPHP\Components\Database::query("SELECT t.*, (t.size * t.times_completed) AS data FROM torrents AS t WHERE leechers > 0 AND times_completed > 0 ORDER BY seeders / leechers ASC, leechers DESC LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "us") {
            $r = \NexusPHP\Components\Database::query("SELECT name, flagpic, COUNT(users.country) as num FROM countries LEFT JOIN users ON users.country = countries.id GROUP BY name ORDER BY num DESC LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "ul") {
            $r = \NexusPHP\Components\Database::query("SELECT c.name, c.flagpic, sum(u.uploaded) AS ul FROM users AS u LEFT JOIN countries AS c ON u.country = c.id WHERE u.enabled = 'yes' GROUP BY c.name ORDER BY ul DESC LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "avg") {
            $r = \NexusPHP\Components\Database::query("SELECT c.name, c.flagpic, sum(u.uploaded)/count(u.id) AS ul_avg FROM users AS u LEFT JOIN countries AS c ON u.country = c.id WHERE u.enabled = 'yes' GROUP BY c.name HAVING sum(u.uploaded) > 1099511627776 AND count(u.id) >= 100 ORDER BY ul_avg DESC LIMIT $limit") or sqlerr();
    ...
    elseif ($type == 4)
    {
        if ($limit == 10 || $subtype == "ul")
        {
            $r = \NexusPHP\Components\Database::query( "SELECT users.id AS userid, username,snatched.upspeed AS uprate, snatched.downspeed AS downrate FROM peers LEFT JOIN snatched ON snatched.userid = peers.userid AND snatched.torrentid = peers.torrent LEFT JOIN users ON users.id = peers.userid ORDER BY uprate DESC LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "dl")
        {
            $r = \NexusPHP\Components\Database::query("SELECT users.id AS userid, username,snatched.upspeed AS uprate, snatched.downspeed AS downrate FROM peers LEFT JOIN snatched ON snatched.userid = peers.userid AND snatched.torrentid = peers.torrent LEFT JOIN users ON users.id = peers.userid ORDER BY downrate DESC LIMIT $limit") or sqlerr();
        ...
        {
            $r = \NexusPHP\Components\Database::query( "SELECT FROM peers LEFT JOIN locations ON peers.ip GROUP BY users.id ORDER BY commentnum DESC LIMIT $limit") or sqlerr();
    ...
    */
    elseif ($type == 5) {
        if ($limit == 10 || $subtype == "mtop") {
            $r = \NexusPHP\Components\Database::query("SELECT users_topics.userid,  users_topics.usertopics, COUNT(posts.id) as userposts FROM (SELECT users.id as userid, COUNT(topics.id) as usertopics from users LEFT JOIN topics ON users.id = topics.userid GROUP BY users.id) as users_topics LEFT JOIN posts ON users_topics.userid = posts.userid GROUP BY users_topics.userid ORDER BY usertopics DESC LIMIT $limit") or sqlerr();
        ...
        if ($reviewenabled == 'yes' && ($limit == 10 || $subtype == "mrev")) {
            $r = \NexusPHP\Components\Database::query("SELECT users.id as userid, COUNT(reviews.id) as num FROM users LEFT JOIN reviews ON users.id = reviews.user GROUP BY users.id ORDER BY num DESC LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "mcmt") {
            $r = \NexusPHP\Components\Database::query("SELECT users.id as userid, COUNT(comments.id) as num FROM users LEFT JOIN comments ON users.id = comments.user GROUP BY users.id ORDER BY num DESC LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "btop") {
            $r = \NexusPHP\Components\Database::query("SELECT topics_posts.topicid, topics_posts.topicsubject, topics_posts.postnum, forums.id as forumid FROM (SELECT topics.id as topicid, topics.subject as topicsubject, COUNT(posts.id) as postnum, topics.forumid FROM topics LEFT JOIN posts ON topics.id = posts.topicid GROUP BY topics.id) as topics_posts LEFT JOIN forums ON topics_posts.forumid = forums.id AND forums.minclassread <= 1 ORDER BY postnum DESC LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "bo") {
            $r = \NexusPHP\Components\Database::query("SELECT * FROM users ORDER BY seedbonus DESC LIMIT $limit") or sqlerr();
        ...
        if ($prolinkpoint_bonus) {
            if ($limit == 10 || $subtype == "pl") {
                $r = \NexusPHP\Components\Database::query("SELECT userid, COUNT(id) AS count FROM prolinkclicks GROUP BY userid ORDER BY count DESC LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "charity") {
            $r = \NexusPHP\Components\Database::query("SELECT * FROM users ORDER BY charity DESC LIMIT $limit") or sqlerr();
        ...
        if ($enabledonation == 'yes') {
            if ($limit == 10 || $subtype == "do_usd") {
                $r = \NexusPHP\Components\Database::query("SELECT id, donated, donated_cny from users where donated > 0 ORDER BY donated DESC, donated_cny DESC LIMIT $limit") or sqlerr();
            ...
            if ($limit == 10 || $subtype == "do_cny") {
                $r = \NexusPHP\Components\Database::query("SELECT id, donated, donated_cny from users where donated_cny > 0 ORDER BY donated DESC, donated_cny DESC LIMIT $limit") or sqlerr();
        ...
        */
        ...
        if ($limit == 10 || $subtype == "mcli") {
            $r = \NexusPHP\Components\Database::query("SELECT agent_allowed_family.family as client_name, COUNT(users.id) as client_num from users RIGHT JOIN agent_allowed_family ON agent_allowed_family.id = users.clientselect GROUP BY clientselect ORDER BY client_num DESC LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "ss") {
            $r = \NexusPHP\Components\Database::query("SELECT stylesheets.name as stylesheet_name, COUNT(users.id) as stylesheet_num from users JOIN stylesheets ON stylesheets.id = users.stylesheet GROUP BY stylesheet ORDER BY stylesheet_num DESC LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "lang") {
            $r = \NexusPHP\Components\Database::query("SELECT language.lang_name as lang_name, COUNT(users.id) as lang_num from users JOIN language ON language.id = users.lang WHERE site_lang=1 GROUP BY lang ORDER BY lang_num DESC LIMIT $limit") or sqlerr();
    ...
    elseif ($type == 7)	// search
    {
        ...
        {
            $r = \NexusPHP\Components\Database::query( "SELECT keywords, adddate from suggest ORDER BY adddate DESC LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "tcmo")
        {
            ...
            $r = \NexusPHP\Components\Database::query("SELECT keywords, COUNT(id) as count FROM suggest WHERE UNIX_TIMESTAMP(adddate) >" . $current_month . " GROUP BY keywords ORDER BY count DESC LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "tlmo")
        {
            ...
            $r = \NexusPHP\Components\Database::query("SELECT keywords, COUNT(id) as count FROM suggest WHERE UNIX_TIMESTAMP(adddate) >" . $last_month_begin . " AND UNIX_TIMESTAMP(adddate) <" . $last_month_end . " GROUP BY keywords ORDER BY count DESC LIMIT $limit") or sqlerr();
        ...
        if ($limit == 10 || $subtype == "tcy")
        {
            ...
            $r = \NexusPHP\Components\Database::query("SELECT keywords, COUNT(id) as count FROM suggest WHERE UNIX_TIMESTAMP(adddate) >" . $current_year . " GROUP BY keywords ORDER BY count DESC LIMIT $limit") or sqlerr();
